...
@Component({
    ...
    <div class="bg-white rounded-[20px] shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-8 flex flex-col justify-start h-full min-h-[190px]"
      ...
      <div class="flex items-center">
        ...
        @if (stat().showPlus) {
          <span class="text-[48px] leading-[50px] font-bold font-bdo text-accent-green ml-3">
            +
          </span>
        }
      </div>
      ...
      <span class="text-[20px] leading-[38px] font-normal font-bdo text-[#0A1642] align-middle">
        {{ stat().label }}
      </span>

      @if (stat().description) {
        <p class="text-[16px] leading-[1.35] text-[#80899D] font-normal font-bdo  mt-2 whitespace-normal break-words">
          {{ stat().description }}
        </p>
      }
    </div>
